            ...
            if (prizeMessagesList) {
                ...
                currentMessages.forEach((currentMsg, index) => {
                    if (!newMessages.some(newMsg => newMsg.title === currentMsg.title && newMsg.uid === currentMsg.uid)) {
                        if (prizeMessagesList.children[index]) {
                ...
                newMessages.forEach((newMsg, index) => {
                    ...
                    if (existingItem) {
                        // 如果消息已存在，更新其内容（如果需要）
                        let displayTitle = newMsg.title;
                        let fullTitle = newMsg.title;
                        try {
                            const parsedTitle = JSON.parse(newMsg.title);
                            if (parsedTitle && parsedTitle.title) {
                                displayTitle = parsedTitle.title;
                                fullTitle = parsedTitle.title; // For tooltip
                            }
                        } catch (e) {
                        ...
                        existingItem.querySelector('.message-content').innerHTML = `
                            <p title="${fullTitle}"><strong>标题:</strong> ${displayTitle}</p>
                        ...
                        listItem.innerHTML = `
                            <div class="message-content">
                                <p title="${fullTitle}"><strong>标题:</strong> ${displayTitle}</p>
                                <p><strong>UID:</strong> ${newMsg.uid}</p>
                                <p><strong>时间:</strong> ${new Date(newMsg.timestamp).toLocaleString()}</p>
                            </div>
                            <div class="actions">
