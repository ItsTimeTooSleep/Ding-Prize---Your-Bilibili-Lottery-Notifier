...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('view-all-prizes').addEventListener('click', () => {
        ...
        chrome.tabs.create({ url: chrome.runtime.getURL('../src/prize_results.html') });
    });
...
function showNotification(message, type = 'info') {
    ...
    notification.className = `notification ${type} show`; // Add type class for styling
    ...
    notificationTimeout = setTimeout(() => {
        ...
        // Remove the notification element after the animation completes
        notification.addEventListener('transitionend', function handler() {
            notification.removeEventListener('transitionend', handler);
            notification.classList.remove('show'); // Now remove 'show' after the hide animation
