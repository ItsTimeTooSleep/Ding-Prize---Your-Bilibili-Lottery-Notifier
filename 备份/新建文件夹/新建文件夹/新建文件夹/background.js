                    ...
                    if (containsPrizeKeywords(message.content, currentPrizeKeywords, currentBlacklistKeywords)) {
                        ...
                        console.log(`[Background] 正在构建 prizeInfo 对象...`);
                         const prizeInfo = {
                             ...
                             messageContent: parsedContent.text || message.content, // 存储解析后的消息内容
                             thumb: parsedContent.thumb || '', // 缩略图
                             timestamp: message.timestamp * 1000 || Date.now(), // 使用消息的timestamp字段（秒级）转换为毫秒，如果不存在则使用当前时间
