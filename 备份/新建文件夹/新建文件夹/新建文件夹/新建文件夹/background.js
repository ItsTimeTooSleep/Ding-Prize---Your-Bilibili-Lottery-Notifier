            ...
            if (msgData.code === 0 && msgData.data && msgData.data.messages) {
                for (const message of msgData.data.messages) {
                    ...
                    if (containsPrizeKeywords(message.content, currentPrizeKeywords, currentBlacklistKeywords)) {
                        ...
                        let extractedRawContent = message.content; // Default to raw message content
                        let thumb = '';
                        ...
                        try {
                            ...
                            // For rawContent, prioritize content field within JSON, then text, then item_text
                            if (parsedContent.content) {
                                try {
                                    const innerParsedContent = JSON.parse(parsedContent.content);
                                    if (innerParsedContent.content) {
                                        extractedRawContent = innerParsedContent.content;
                                    } else {
                                        extractedRawContent = parsedContent.content;
                                    }
                                } catch (innerE) {
                                    extractedRawContent = parsedContent.content;
                                }
                            } else if (parsedContent.text) {
                                extractedRawContent = parsedContent.text;
                            } else if (parsedContent.item_text) {
                                extractedRawContent = parsedContent.item_text;
                            }
                            ...
                            extractedRawContent = message.content;
                        }
                        ...
                        console.log(`[Background] extractedRawContent:`, extractedRawContent);
                        console.log(`[Background] 正在构建 prizeInfo 对象...`);
                         const prizeInfo = {
                             ...
                             messageContent: extractedRawContent, // Use the refined raw content
                             thumb: thumb, // Use the extracted thumb
                             timestamp: message.timestamp * 1000 || Date.now(), // Use message's timestamp or current time
                             rawContent: extractedRawContent // Store the refined raw content
                         };
