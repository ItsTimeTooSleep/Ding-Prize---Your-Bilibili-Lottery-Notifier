...
async function sendNotification(message, type = 'success') {
    const notificationTitle = 'BiliPrize Alert'; // 浏览器通知的固定标题
    ...
    chrome.runtime.sendMessage({ type: "showNotification", notificationType: type, message: message });
...
async function checkBiliMessages() {
    ...
    const sessionPromises = sessionTypes.map(async (type) => {
        try {
            ...
            if (data.code === 0 && data.data && data.data.session_list) {
                ...
                sendNotification('BiliPrize Alert', 'B站登录状态失效，请重新登录！');
    ...
    for (const session of allSessions) {
                ...
                for (const message of msgData.data.messages) {
                    ...
                    if (containsPrizeKeywords(message.content, currentPrizeKeywords, currentBlacklistKeywords)) {
                        ...
                        try {
                            ...
                            console.warn("Failed to parse message content as JSON:", message.content);
                            parsedContent = { text: message.content }; // Fallback if not JSON
                        }

                        console.log(`Debug: session.talker_name for prizeInfo: ${session.talker_name}`);
                         const prizeInfo = {
                             title: (parsedContent.item_text || parsedContent.text || message.content)
                                        .replace(/^content: "?/, '')
                                        .replace(/"?$/, '')
                                        .replace(/\{"title":"/, '') // 移除 {"title":"
                                        .substring(0, 50) + '...', // 尝试从不同字段获取标题并去除 "content" 和引号
                             uid: session.talker_id,
                             author: session.talker_name || '未知发送者',
                             rawContent: message.content // 存储原始消息内容以备用
                         };
                         newPrizeMessages.push(prizeInfo);
                        console.log(`Prize message identified: "${message.content}" matched by keywords: ${currentPrizeKeywords.filter(keyword => message.content.includes(keyword)).join(', ')}`);
                        settings.processedMessages.push(JSON.parse(message.content).id);
                        sendNotification(
                            'BiliPrize Alert - 发现中奖消息！',
                            `来自 ${session.talker_name} 的消息: ${message.content.substring(0, 100)}...`
                        );
                        newPrizeFound = true; // 标记发现新的中奖消息
                    } else if (currentBlacklistKeywords.some(blackword => message.content.includes(blackword))) {
                        // console.log(`Message filtered by blacklist: "${message.content}" contains a blacklisted keyword.`);
                    }
                }
            } else {
        ...
        // await new Promise(resolve => setTimeout(resolve, 200)); // 避免频繁请求
    }
    ...
    const uniqueNewPrizeMessages = newPrizeMessages.filter(newMsg => {
        ...
        return !existingPrizeMessages.some(existingMsg =>
